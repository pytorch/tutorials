...
if [[ $USE_CACHE== "y" ]]; then
  ...
  if [[ "${JOB_BASE_NAME}" == *worker_* ]]; then
    ...
    mkdir -p circleci_build_cache
    rm -f circleci_build_cache/tutorial_files_included.txt
    for f in ${FILES_TO_RUN[@]}
    do
      echo "${f}.py" >> circleci_build_cache/tutorial_files_included.txt
    done
    ...
    # cp to cache too
    cp worker_${WORKER_ID}.7z circleci_build_cache/
