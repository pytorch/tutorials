# -*- coding: utf-8 -*-
"""
Knowledge Distillation (KD) Tutorial
===============================
**Author**: `Alexandros Chariton <https://github.com/AlexandrosChrtn>`_
""" 

######################################################################
# This tutorial focuses on the inclusion of additional loss function along with the traditional classification loss, Cross-Entropy. We will be using a popular image dataset, CIFAR-10 and the rest of the code should be easy to run with
# a simple GPU (4GB of memory should be more than enough). We will be running a number of experiments focused at improving the accuracy of a lightweight neural network, using a more powerful network as a teacher.
...
# A common practice in NNs is to normalize the input. This is done for multiple reasons like avoiding saturation in common activation functions or increased numerical stability. Our Normalization consists of subtracting the mean and dividing by the standard deviation, along each channel.
# The tensors ``mean=[0.485, 0.456, 0.406]`` and ``std=[0.229, 0.224, 0.225]`` were precomputed and they are the mean / std of each channel in the predefined subset of CIFAR-10 meant to be the train set. Notice how we use these numbers to the test set too and we do not compute the mean / std of the test set from scratch.
...
# Deeper NN class to be used as teacher
...
# Instantiate the lightweight nn model
torch.manual_seed(42)
nn_light = LightNN(num_classes=10)

######################################################################
# We instantiate one more lightweight NN model to compare their performances.
...
# Print the norm of the first layer of the initial lightweight nn model
print("Norm of 1st layer of nn_light:", torch.norm(nn_light.features[0].weight).item())
# Print the norm of the first layer of the new lightweight nn model
...
# We do not dive deeper into KD as the purpose of this tutorial is to showcase how one would handle multiple loss functions.
#
...
#    :align: center
#    
#    Distillation loss is calcluated from the logits of the networks. It only returns gradients to the student
#
...
def train_knowledge_distillation(teacher, student, train_loader, epochs, learning_rate, T, soft_target_loss_weight, ce_loss_weight, device):
    ...
    teacher.to(device)
    student.to(device)
    teacher.eval()  # Teacher set to eval mode
    ...
    for epoch in range(epochs):
        ...
        for inputs, labels in train_loader:
            ...
            # Calculate the soft targets loss. Scaled by T**2 as suggested by the authors of KD
...
# Compare the student test accuracy with and without the teacher, after introducing simple KD.
print(f"Teacher accuracy: {test_accuracy_deep:.2f}%")
...
# It is common practice in NNs to include additional loss functions to the main classification objectives to achieve goals like better generalization.
...
# We do not have to train the modified DNN from scratch of course, we just load its weights from the trained instance
...
# Initialize a ModifiedLightNN with the same seed as our other lightweight instances. This will be trained from scratch to examine the effectiveness of cosine loss minimization.
torch.manual_seed(42)
...
# It is ``1024`` for the teacher too, because we made it so with ``avg_pool1d`` from ``2048``. The loss applied here only affects the weights of the student prior to the loss caclulation. In other words it does not affect the classifier of the student.
...
def train_cosine_loss(teacher, student, train_loader, epochs, learning_rate, hidden_rep_loss_weight, ce_loss_weight, device):
    ...
    for epoch in range(epochs):
        ...
        for inputs, labels in train_loader:
            ...
            # target is a vector of ones. From the CosineEmbeddingLoss formula above we can see that is the case where loss minimization leads to cosine similarity increase, which is desirable.
...
######################################################################
# In this case we could easily include both KD and cosine loss minimization in the same function. It is common to combine methods to achieve better performance in teacher-student paradigms.
...
#    :align: center
#    
#    The trainable layer matches the shapes of the intermediate tensors and MSE is properly defined
#
...
class ModifiedLightNNRegressor(nn.Module):
    def __init__(self, num_classes=10):
        ...
        # Include an extra NN regressor (linear projection)
...
# Including the extra network is the idea behind hint based distillation, proposed in the original FitNets paper.
