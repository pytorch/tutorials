...
class. You can run this notebook locally, on
`Colab <https://colab.research.google.com/>`__, or on your preferred
cloud service.
...
# terminology: \* You will sometimes see a 1-dimensional tensor called a
# *vector.* \* Likewise, a 2-dimensional tensor is often referred to as a
# *matrix.* \* Anything with more than two dimensions is generally just
# called a tensor.
# 
...
# 
# **Note: The following cell throws a run-time error. This is
# intentional.**
# 
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# 
# *(Note: If you are familiar with broadcasting semantics in NumPy
...
######################################################################
# Look closely at the values of each tensor above: \* The multiplication
# operation that created ``b`` was broadcast over every “layer” of ``a``.
# \* For ``c``, the operation was broadcast over ever layer and row of
# ``a`` - every 3-element column is identical. \* For ``d``, we switched
# it around - now every *row* is identical, across layers and columns.
# 
...
######################################################################
# This is a small sample of For more details and the full inventory of
...
# ``is_available()`` method.
# 
# **Note: If you do not have a CUDA-compatible GPU and CUDA drivers
...
######################################################################
# *(Note: The ``(6 * 20 * 20,)`` argument in the final line of the cell
