...
# ``__getitem__``. This is memory effecient because all the images are not
# stored in the memory at once but read as required.
#
# Sample of our dataset will be a dict
# ``{'image': image, 'landmarks': landmarks}``. Our datset will take an
# optional parameter ``transform`` so that any required transform can be
...
# ----------
#
# One issue we can see from above is that samples are not of the same
# size. Here is where we will need the data transforms.
#
...
# called. For this we just need to implement ``__call__`` method. We can
# then use the transform like this:
#
# ::
#
...
class Rescale(object):
    ...
    Args:
        output_size (tuple or tuple): Desired output size. If tuple, output is
            matched to output_size. If int, smaller of output edges is matched
            to output_size.
    """
...
# transforms. We can iterate over the created dataset with a simple for
# loop as before.
#
# To summarize, every time dataset is sampled:
#
...
######################################################################
# However, we are loosing a lot of features by using a simple for loop to
...
def show_landmarks_batch(sample_batched):
    ...
    images_batch, landmarks_batch = sample_batched[
        'image'], sample_batched['landmarks']
