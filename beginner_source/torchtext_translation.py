...
def build_vocab(train_iter, de_tokenizer, en_tokenizer):
  de_counter = Counter()
  en_counter = Counter()
  for (raw_de, raw_en) in train_iter:
    de_counter.update(de_tokenizer(raw_de))
    en_counter.update(en_tokenizer(raw_en))
  return Vocab(de_counter, specials=['<unk>', '<pad>', '<bos>', '<eos>']), Vocab(en_counter, specials=['<unk>', '<pad>', '<bos>', '<eos>'])

de_vocab, en_vocab = build_vocab(train_iter, de_tokenizer, en_tokenizer)

def data_process(data_iter):
  data = []
  for (raw_de, raw_en) in data_iter:
