...
#
# Both resampling methods use bandlimited sinc interpolation as described
# `here <https://ccrma.stanford.edu/~jos/resample/>`__ to compute signal
# values at arbitrary time steps. The implementation involves convolution,
# so we can take advantage of GPU / multithreading for performance
# improvements. When using resampling in multiple subprocesses, such as
# data loading with multiple worker processes, your application might
# create more threads than your system can handle efficiently. Setting
# ``torch.set_num_threads(1)`` might help in this case.
#
...
librosa_resampled_waveform = torch.from_numpy(librosa.resample(waveform.squeeze().numpy(), sample_rate, resample_rate, res_type='kaiser_best')).unsqueeze(0)
...
librosa_resampled_waveform = torch.from_numpy(librosa.resample(waveform.squeeze().numpy(), sample_rate, resample_rate, res_type='kaiser_fast')).unsqueeze(0)
...
# in ``torchaudio``.
#
# To elaborate on the results: - a larger ``lowpass_filter_width`` results
# in a larger resampling kernel, and therefore increases computation time
# for both the kernel computation and convolution - using
# ``kaiser_window`` results in longer computation times than the default
# ``sinc_interpolation`` because it is more complex to compute the
# intermediate window values - a large GCD between the sample and resample
# rate will result in a simplification that allows for a smaller kernel
# and faster kernel computation.
#
...
for label in configs:
  ...
  f_time = %timeit -o F.resample(waveform, sample_rate, resample_rate, lowpass_filter_width=64)
  resampler = T.Resample(sample_rate, resample_rate, lowpass_filter_width=64).to(waveform.dtype)
  t_time = %timeit -o resampler(waveform)
  times.append([None, 1000 * f_time.best, 1000 * t_time.best])
  rows.append(f"sinc (width 64)")

  # sinc 6 zero-crossings
  resampler = T.Resample(sample_rate, resample_rate, lowpass_filter_width=16).to(waveform.dtype)
  f_time = %timeit -o F.resample(waveform, sample_rate, resample_rate, lowpass_filter_width=16)
  t_time = %timeit -o resampler(waveform)
  times.append([None, 1000 * f_time.best, 1000 * t_time.best])
  rows.append(f"sinc (width 16)")

  # kaiser best
  resampler = T.Resample(sample_rate, resample_rate, lowpass_filter_width=64, rolloff=0.9475937167399596,
                        resampling_method="kaiser_window", beta=14.769656459379492).to(waveform.dtype)
  lib_time = %timeit -o librosa.resample(waveform_np, sample_rate, resample_rate, res_type="kaiser_best")
  f_time = %timeit -o F.resample(waveform, sample_rate, resample_rate, lowpass_filter_width=64, rolloff=0.9475937167399596, \
                    resampling_method="kaiser_window", beta=14.769656459379492)
  t_time = %timeit -o resampler(waveform)
  times.append([1000 * lib_time.best, 1000 * f_time.best, 1000 * t_time.best])
  rows.append(f"kaiser_best")

  # kaiser fast
  resampler = T.Resample(sample_rate, resample_rate, lowpass_filter_width=16, rolloff=0.85,
                        resampling_method="kaiser_window", beta=8.555504641634386).to(waveform.dtype)
  lib_time = %timeit -o librosa.resample(waveform_np, sample_rate, resample_rate, res_type="kaiser_fast")
  f_time = %timeit -o F.resample(waveform, sample_rate, resample_rate, lowpass_filter_width=16, rolloff=0.85, \
