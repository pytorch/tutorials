...
def train(model: nn.Module) -> None:
    ...
    for batch_num, i in enumerate(range(0, train_data.size(0) - 1, bptt)):
        data, targets = get_batch(train_data, i)
        batch_seq_len = data.size(0)
        if batch_seq_len != bptt:  # last batch may be truncated
            src_mask = src_mask[:batch_seq_len, :batch_seq_len]
        ...
        if batch_num % log_interval == 0 and batch_num > 0:
            ...
            print(f'| epoch {epoch:3d} | {batch_num:5d}/{num_batches:5d} batches | '
...
def evaluate(model: nn.Module, eval_data: Tensor) -> float:
    ...
    with torch.no_grad():
        for i in range(0, eval_data.size(0) - 1, bptt):
            ...
            batch_seq_len = data.size(0)
            if batch_seq_len != bptt:
                src_mask = src_mask[:batch_seq_len, :batch_seq_len]
            output = model(data, src_mask)
            output_flat = output.view(-1, ntokens)
            total_loss += batch_seq_len * criterion(output_flat, targets).item()
