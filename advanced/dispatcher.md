...
This tutorial is deprecated as of PyTorch 2.4. Please see [PyTorch Custom Operators](custom_ops_landing_page.html#custom-ops-landing-page)
...
If in your [custom operator code](torch_script_custom_ops) you find yourself
...
[register an operator](torch_script_custom_ops) and how to write
a [custom autograd function](cpp_autograd).
...
1. The `Autograd` or `ADInplaceOrView` boxed kernels registered above
...
[here](https://github.com/pytorch/pytorch/blob/master/aten/src/ATen/native/README.md)
...
autograd kernel (similar to what's described in the [custom autograd function](cpp_autograd) tutorial)!
...
dispatcher in [extending dispatcher for a new backend](extend_dispatcher).
...
[autocast-enabled contexts](https://pytorch.org/docs/stable/amp.html#torch.cuda.amp.autocast).
...
[eligibility policy](https://pytorch.org/docs/stable/amp.html#op-eligibility) for natively autocasted ops).
...
[cast lists](https://pytorch.org/docs/master/amp.html#op-specific-behavior).
General guidance:
...
If your custom op is autograd-enabled, you only need to write and register
