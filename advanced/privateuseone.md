...
by `TORCH_LIBRARY_IMPL` API described in [Registering a Dispatched Operator in C++](dispatcher). This involves
several situations:
...
1. Register kernels from `torch::autograd::Function` to the dispatcher by `AutogradPrivateUse1`, if it is necessary for
...
1. Register kernels which want to support [automatic mixed precision (AMP)](https://pytorch.org/docs/stable/amp.html) and
...
One example about `PrivateUse1` integration is [Ascend NPU](https://github.com/ascend/pytorch).
