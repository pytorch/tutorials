...
# creating a copy of the model by Loading the original model because quantization api changes the model inplace and we want
...
model_to_quantize = model.load_state_dict(
    torch.load(
        model_data_filepath + 'word_language_model_quantize.pth',
        map_location=torch.device('cpu')
        )
