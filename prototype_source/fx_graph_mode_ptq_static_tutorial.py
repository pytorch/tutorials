...
The advantage of FX graph mode quantization is that as long as the model can be symbolically traced, 
we can perform quantization fully automatically on the model. 

We have a separate tutorial for FX Graph Mode Post Training Dynamic Quantization(todo).	
...
# -----------------------------------------
# ``convert_fx`` takes a calibrated model and prodcuces a quantized model.
