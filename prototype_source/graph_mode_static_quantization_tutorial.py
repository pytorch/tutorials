...
We have a separate tutorial for Graph Mode Post Training Dynamic Quantization.
...
# ---------------------
# Currently PyTorch only has eager mode quantization: [Post Training Static Quantization in Eager Mode](https://pytorch.org/tutorials/advanced/static_quantization_tutorial.html).
#
...
# These steps are identitcal to https://pytorch.org/tutorials/advanced/static_quantization_tutorial.html, 
#
# Download dataset:
#
# .. code::
...
# Download model from `https://github.com/pytorch/vision/blob/master/torchvision/models/resnet.py#L12` and rename it to
...
# 3. ``convert_jit`` takes a calibrated model and prodcuces a quantized model.
#
# If ``debug`` is False(default option), ``convert_jit`` will:
#
# - calculate quantization parameters using the observers in the model
# - insert quantization ops like ``aten::quantize_per_tensor`` and ``aten::dequantize`` to the model, and remove the observer modules after that.
...
# easily determine the sensitivity towards quantization of different modules in a model: [FILL IN NUMERIC SUITE OSS TUTORIAL]
#
...
# More instructions on how to debug TorchScript model can be found in https://pytorch.org/docs/stable/jit.html#debugging.
#
#
