...
# doing ``-`` as ``tmp0`` is ``long``.
# We can easily match ``-`` and ``max_propagate_nan`` in C++ kernel with
# ``ops.neg`` and ``ops.maximum`` in IR node respectively.
#
# Now we successfully find that the root cause is the implementation of
# ``ops.neg`` in ``cpp`` codegen, which silently changes the data type when doing ``neg``.
#
# Accuracy debugging
# ~~~~~~~~~~~~~~~~~~
#
# Otherwise, if the model runs with other errors or accuracy problem, you can
# use the PyTorch debugging tool called `Minifier <https://pytorch.org/functorch/stable/notebooks/minifier.html>`_.
#
...
# ---------------------
#
# Within this section, we will demonstrate the process of conducting performance
# analysis for a model that has been compiled using the Inductor CPU backend.
# In the example below, we benchmark a Hugging Face Transformer model
# ``MobileBertForQuestionAnswering`` with both the eager mode and the Inductor
# graph mode.
# The execution time and the speedup ratio of Inductor are printed after the
# benchmark.
...
#
#
# Next, let's dive deep into the performance at the operation level to understand
...
# Inductor CPU backend has the support to report the time of the fusion kernels
# to the profiler with the ``enable_kernel_profile`` configuration option:
...
# ``aten::mul``, ``aten::clamp_min``, ``aten::bmm``].
# Comparing with the inductor model profiling table, we notice an
# ``mkl::_mkl_linear`` entry and multiple fused kernels in the form
# ``graph_0_cpp_fused_*``. They are the major
# optimizations that the inductor model is doing. Let us discuss them
# separately.
#
# (1) Regarding ``mkl::_mkl_linear``: You may notice the number of calls
# to this kernel is 362, which is exactly the same as ``aten::linear`` in the
# eager model profiling table.
# The CPU total of ``aten::linear`` is 376.888ms, while it is 231.573ms for
# ``mkl::_mkl_linear``. This suggests a ~1.63x for the "linear" part.
# The speedup mainly comes from `packing the weight tensor to block memory
# format <https://www.intel.com/content/www/us/en/docs/onemkl/developer-reference-c/2023-1/cblas-gemm-pack-002.html>`_
...
# that the speed up for the other memory-intensive ops is around 3.94x.
# Let's read the generated code to understand how the inductor achieves
# this impressive optimization. You can find the generated code by 
...
# good performance. To get a more intuitive feeling about this optimization,
# we can infer the sizes and stride of the inputs and further benchmark this
...
# Minifier.
# Firstly determine which component the failure occurs in and then try to
# generate the smallest snippet of code that can reproduce the failure.
#
# When the performance with Inductor is better than that of eager mode, we
# provide a solid analytical method for performance profiling.
# We show how to find the time-consuming hotspot with PyTorch Profiler and
# figure out the operator-level or kernel-level reason to explain the
# phenomenon.
