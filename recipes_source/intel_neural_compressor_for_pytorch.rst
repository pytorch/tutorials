...
focus due to significant performance boost. One of essential concerns on adopting
low precision is how to easily mitigate the possible accuracy loss and reach
...
*This tutorial mainly focus on quantization part. As for how to use Intel® Neural
Compressor to do pruning and distillation, please refer to corresponding documents
in Intel® Neural Compressor github repo.*
...
Minor code changes are required for users to get start with Intel® Neural Compressor*
quantization API. Both PyTorch fx graph mode and eager mode are supported.

Intel® Neural Compressor takes FP32 model and YAML configuration file as two inputs.
To construct the quantization process, users can either specify below settings via YAML or
python APIs: 
...
Intel® Neural Compressor supports some popular dataloaders and evaluation metrics.
User could setup *Calibration Dataloader*, *Evaluation Dataloader*, and *Evaluation
Metric* through yaml configuration file for using those built-in components.
For how to use it in yaml file, user could refer to `Built-in Datasets
<https://github.com/intel/neural-compressor/blob/master/docs/dataset.md>`_

If user wants to use self-developed dataloader or evaluation metric, Intel® Neural
Compressor* supports this by registration of customized dataloader/metric using python code.

The yaml configuration file format please refer to `yaml template
...
comments in a line above.

Model
^^^^^

In this tutorial, LeNet model is used to demonstrate how to deal with *Intel® Neural Compressor*.
...
int8 model which meets predefined accuracy goal.
...
the evaluation method and `1%` relative accuracy loss is set as the accuracy target for auto
tuning. Intel® Neural Compressor will traverse all possible quantization config combinations on per-op level to find out the optimal
int8 model who reaches the predefined accuracy target.
...
In above example, user implements a `class` which contains `update()` and `result()` function
...
Compressor* supports quantization-aware training with accuracy-driven automatic tuning mechanism.
...
Intel® Neural Compressor* supports directly yielding int8 model with dummy dataset for performance
benchmarking purpose.

Below is an example of how to quantize a simple network on PyTorch
`FX graph mode <https://pytorch.org/docs/stable/fx.html>`_ with dummy dataset.
...
User could know how many ops get quantized from log printed by Intel® Neural Compressor
like below:
...
The first file contains quantization config of each op, the second file contains int8 weights and
zero point and scale info of activations.

Deployment
~~~~~~~~~~

User could use below code to load quantized model and then do inference or performance benchmark.
