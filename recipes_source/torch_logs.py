...
# Determine the device — use CPU fallback if no suitable GPU is available
if torch.cuda.is_available() and torch.cuda.get_device_capability() >= (7, 0):
    device = "cuda"
else:
    device = "cpu"
    print("CUDA with compute capability 7.0+ not found. Running with CPU inductor backend.")
