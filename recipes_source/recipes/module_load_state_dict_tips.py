...
1.   The `mmap` kwarg on `torch.load`
2.   The `torch.device()` context manager
3.  The `assign` kwarg on `nn.Module.load_state_dict()`
...
# they were tagged with (unless this behavior is overriden using the
...
#
# The `mmap` kwarg to `torch.load` attempts to solve the above two problems by
# using an [mmap call](https://man7.org/linux/man-pages/man2/mmap.2.html) on
# the checkpoint, so that tensor storages are memory-mapped and when they are
# fetched from disk to memory is managed by the OS.
#
# Next, we consider the instantiation of the module.
...
# TODO: maybe include recent PR to override part 2 with TorchFunctionMode
#
...
# In order to avoid this, we can pass the `assign=True` kwarg to
# `load_state_dict()`.
...
###############################################################################
# To recap, in this tutorial, we learnt about `torch.load(mmap=True)`, the
